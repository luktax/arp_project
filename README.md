# arp_project
https://github.com/luktax/arp_project
...
-channel of comunication with the father.
 (write, read)
# loop
...
WATCHDOG PROCESSy
(It was not possible to create a window with ncurses for reason I couldn't understand, I didn't manage to solve the problem in time. Everything works correctly but the moment I try to create a window, the watchdog code don't even start. I'll try to solve it for the third assignment)
# initialization
...
-I wrote a simple function in the blackboard loop, to check if the robot grab a target, if it is the case it reorders the obstacles arrays and notify the map. (line of code: Blackboard.c 339-365)
...
# news
-OBS: now the targets are numbered and can be grabbed by the drone and every time it happens a new target is generated
-TGS: now the obstacles disappear and appear every 5 seconds randomly in the map.
-system.log: every process write a log message every time it enters an important area of code. 
-process_pid.log: every process register his name and his pid on this file
